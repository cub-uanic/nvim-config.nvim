...
local function exists(path) return vim.fn.filereadable(path) == 1 end
...
local function unquote(s)
  s = trim(s)
  if (s:sub(1, 1) == '"' and s:sub(-1) == '"') or (s:sub(1, 1) == "'" and s:sub(-1) == "'") then return s:sub(2, -2) end
  return s
end
...
local function parse_env(lines)
  ...
  for _, line in ipairs(lines or {}) do
    ...
    if line ~= '' then
      ...
      if k and v then env[k] = unquote(v) end
    end
  end
  return env
end
...
local function yaml_get(lines, key)
  ...
  for _, line in ipairs(lines or {}) do
    ...
    if k == key and v and v ~= '' then return unquote(v) end
  end
  return nil
end
...
local function infer_from_dancer_config_yml(root)
  ...
  if driver and database then
    ...
    local hp = host
    if port and port ~= '' then hp = ('%s:%s'):format(host, port) end
...
local function infer_from_database_yml(root)
  ...
  if #dev_lines == 0 then return nil end
  ...
  if adapter and database then
    ...
    if scheme == 'sqlite' then return { key = 'project', dsn = ('sqlite:%s'):format(database), source = 'config/database.yml(sqlite)' } end
...
local function merge_dbs(new_dbs)
  if not new_dbs or not new_dbs.key or not new_dbs.dsn then return false end

  vim.g.dbs = vim.g.dbs or {}

  -- don't overwrite
  if vim.g.dbs[new_dbs.key] and vim.g.dbs[new_dbs.key] ~= '' then return false end
...
function M.load_for_cwd(opts)
  ...
  if not found then found = infer_from_dancer_config_yml(root) end
  if not found then found = infer_from_database_yml(root) end
  ...
  if found then
    ...
    if opts.notify then notify('dadbod: no project DB config found (.env/config.yml/config/database.yml)', vim.log.levels.WARN) end
    return false
  end
end
