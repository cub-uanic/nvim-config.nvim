return {
  {
    ...
    opts = function()
      ...
      local function is_under(path, root) return root and path:sub(1, #root) == root and (path:sub(#root + 1, #root + 1) == '/' or #path == #root) end
      ...
      local function proj_path_with_icon()
        ...
        if vim.api.nvim_win_get_width(0) < 120 then shown = vim.fn.pathshorten(shown) end

        -- Add icon
        local ok, devicons = pcall(require, 'nvim-web-devicons')
        if ok then
          local icon = devicons.get_icon(path, nil, { default = true })
          if icon and icon ~= '' then shown = icon .. ' ' .. shown end
        end
